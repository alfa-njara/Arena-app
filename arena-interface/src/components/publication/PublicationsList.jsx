...
const PublicationsList = () => {
  ...
  const filteredPublications = publications.filter((pub) => {
    ...
    if (activeFilter === "Popular") {
      const scoreA = (a.total_views || 0) + (a.total_visits || 0) * 3 + (a.total_favorites || 0) * 5;
      const scoreB = (b.total_views || 0) + (b.total_visits || 0) * 3 + (b.total_favorites || 0) * 5;
          ...
          {filteredPublications.map((pub, idx) => {
              ...
              return (
                <div key={idx} className="col-xl-3 col-lg-4 col-md-6">
                  <div className={`pub-glass-card h-100 ${pub.is_premium ? 'premium-card' : ''}`} onClick={() => setSelectedCompany(pub)}>
                    {pub.is_premium && (
